...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const howToSchema = (headers && headers.length >= 3) ? {
        ...
        "step": headers.map((h: any, i: number) => ({
            ...
            "name": h.text,
            "text": h.text,
            "url": `https://expertbornerecharge.com/blog/${slug}#${h.id}`
        }))
    } : null;

    return (
        <main className="min-h-screen bg-white text-neutral-900 font-sans selection:bg-blue-100 selection:text-blue-900 pt-20">
             <script
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold overflow-hidden relative">
                                    {(post.author_name || 'E')[0]}
                                      {/* Verified Badge */}
                                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm">
                                        <div className="bg-blue-500 rounded-full w-3 h-3 border-2 border-white"></div>
                                    </div>
                                </div>
                                <div>
                                    <Link href={post.author_slug ? `/author/${post.author_slug}` : "#"} className="font-bold text-neutral-900 hover:text-amber-600 transition-colors">{post.author_name || 'Expert Borne Recharge'}</Link>
                                    <p className="text-xs text-neutral-500">Expert IRVE</p>
                                </div>
                            </div>
                        </div>
