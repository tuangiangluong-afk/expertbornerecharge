            ...
            <div className="container mx-auto px-4">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <p className="text-xl text-neutral-600">
                        Guides, actualités et conseils d'experts pour réussir votre installation de borne IRVE.
                    </p>
                </div>
                    ...
                    {posts.map((post: any) => (
                        <Link key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-neutral-100 h-full">
                             <div className="relative h-56 w-full overflow-hidden">
                                ...
                                {post.category && (
                                    <span className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-blue-900 text-xs font-bold px-3 py-1 rounded-full border border-blue-100">
                                        {post.category.name}
                                    </span>
                                )}
                             </div>
